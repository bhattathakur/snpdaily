...
data=[go.Candlestick(x=df['Date'],open=df['Open'],high=df['High'],low=df['Low'],close=df['Close'],name=f'{ticker}-CLOSE {close:>10}')]
...
vwap_data=go.Scatter(x=df['Date'],y=df['VWAP'],mode='lines',name=f'YTD-VWAP {vwap:>18.2f}',line=dict(color='orange',width=2))
#container 1
